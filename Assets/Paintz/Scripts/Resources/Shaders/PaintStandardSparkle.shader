Shader "Paint/PaintStandardSparkle" {
	Properties{
		...
		_Intensity("Intensity", Range(0,200)) = 50
...
#include "UnityCG.cginc"
	...
	void surf(Input IN, inout SurfaceOutputStandard o) {
		...
		pc.xyz = lerp(pc.xyz, _SplatColor1.xyz, splatMask.x);
		pc.xyz = lerp(pc.xyz, _SplatColor2.xyz, splatMask.y);
		pc.xyz = lerp(pc.xyz, _SplatColor3.xyz, splatMask.z);
		pc.xyz = lerp(pc.xyz, _SplatColor4.xyz, splatMask.w);

		float at = pc.x + pc.y + pc.z;
		if (at > 0)
		{
			//fixed3 sparklemap = tex2D(_NoiseTex, IN.uv_MainTex * _Scale);
			//sparklemap -= half3(0.5, 0.5, 0.5);
			//sparklemap = normalize(normalize(sparklemap) + IN.worldNormal);
		...
		c.xyz = lerp(c.xyz, pc.xyz, _Transparency);
